...
public class DQIDataWriter {
	...
	private static final Logger logger = Logger.getLogger(DQIDataWriter.class);
	 ...
	 */
	public void write(SetupParametersVO setupParametersVO, Map<String, DataCollectionVO> map,
			List<QATeamMemberDetailsVO> qAlist, BaseMeasuresVO baseMeasuresVO) throws FileNotFoundException {
		DQIVariables.getInstance().setWriteExcel(false);
		try {
			...
			cell14a.setCellValue(DQIVariables.getInstance().getdQIStoryPoints());
			...
			for (String name : map.keySet()) {
				...
				if (name != null) {
					...
					datarow17.createCell(0).setCellValue(DQIVariables.getInstance().getdQISprintNumber());
					datarow17.getCell(0).setCellStyle(style1);
					datarow17.createCell(1).setCellValue(DQIVariables.getInstance().getdQIMonthYear());
			...
			for (String name : map.keySet()) {
				...
				if (name != null) {
					...
					datarowx.createCell(0).setCellValue(DQIVariables.getInstance().getdQISprintNumber());
					datarowx.getCell(0).setCellStyle(style1);
					datarowx.createCell(1).setCellValue(DQIVariables.getInstance().getdQIMonthYear());
			...
			for (QATeamMemberDetailsVO name3 : qAlist) {
				...
				datarow23.createCell(1).setCellValue(DQIVariables.getInstance().getdQIMonthYear());
			...
			System.err.println("DQIVariables.getInstance().getdQIfinalSheetPath()= "+DQIVariables.getInstance().getdQIfinalSheetPath());

			finalworkbook = workbook;
			System.out.println();
			workbook.close();
			outputStream.close();
			DQIVariables.getInstance().resetData();
			logger.info("Data written successfully in excel");

		} catch (IOException e) {

			//DQIVariables.getInstance().setExceptionOccurred(true);
			logger.error("Exception in DQIData writer class ", e);
		} catch (Exception e) {
